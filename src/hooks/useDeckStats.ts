...
export function calculateDeckStats(deckCards: DeckCard[]): DeckStats {
  ...
  const artistCount = deckCards
    .filter((c) => Array.isArray(c.type) ? c.type[0] === 'Artist' : c.type === 'Artist')
    .reduce((sum, c) => sum + c.quantity, 0);

  const eventCount = deckCards
    .filter((c) => Array.isArray(c.type) ? c.type[0] === 'Event' : c.type === 'Event')
    .reduce((sum, c) => sum + c.quantity, 0);

  const itemCount = deckCards
    .filter((c) => Array.isArray(c.type) ? c.type[0] === 'Item' : c.type === 'Item')
    .reduce((sum, c) => sum + c.quantity, 0);
...
export function useDeckStats(deckCards: DeckCard[]): DeckStats {
  return useMemo(() => calculateDeckStats(deckCards), [deckCards]);
}
